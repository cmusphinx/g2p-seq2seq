...
def _evaluate_once(checkpoint_path,
  ...
  session_creator = monitored_session.ChiefSessionCreator(
      ...
      config=config)
  #Tracer()()
  print('FINAL_OPS_HOOK')
  final_ops_hook = basic_session_run_hooks.FinalOpsHook(
      final_ops, final_ops_feed_dict)
  hooks.append(final_ops_hook)
  print('HOOKS AFTER APPENDING FINAL_OPS: ', hooks)

  with monitored_session.MonitoredSession(
      session_creator=session_creator, hooks=hooks) as session:
    print('MONITORED SESSION')
    if eval_ops is not None:
      print('EVAL_OPS IS NOT NONE')
