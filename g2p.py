...
tf.app.flags.DEFINE_boolean("count_wer", False,
                            "Set to True for count Word Error Rate.")
...
def train():
  """Train a gr->ph translation model using WMT data."""
  # Prepare G2P data.
  ...
  with tf.Session() as sess:
    ...
    while True:
      ...
      if current_step % FLAGS.steps_per_checkpoint == 0:
        ...
        if len(previous_losses) > 24 and previous_losses[-25:-24] <= min(previous_losses[-25:]):
          break
...
def count_wer():
  with tf.Session() as sess:
    ...
    test_graphemes = open(test_gr_path).readlines()
    test_phonemes = open(test_ph_path).readlines()
    ...
    for i in range(len(test_graphemes)-1):
      ...
      print(word, " > ", test_phonemes[i], " | ", model_assumption)
      if (i > 0) and (i % 10 == 0):
          print("WER = ", k/i)
          print("Accuracy = ", (1-(k/i)) )
    print("WER : ", k/len(test_graphemes) )
    print("Accuracy : ", (1-k/len(test_graphemes)) )
  ...
  if FLAGS.decode:
    ...
    if FLAGS.train:
      ...
      if (not FLAGS.valid) and (not FLAGS.test):
        ...
        test_dic = open(FLAGS.test_dic).readlines()
        for i, word in enumerate(source_dic):
          if i % 20 == 0:
            valid_dic.append(word)
          else: train_dic.append(word)
      elif not FLAGS.test:
        dev_dic = open(FLAGS.dev_dic).readlines()
        for i, word in enumerate(source_dic):
          if i % 10 == 0:
            test_dic.append(word)
          else: train_dic.append(word)
      else:
        dev_dic = open(FLAGS.valid).readlines()
